 ...
 * 1. Creating a III with metrics reporting enabled
...
async function testWorkerMetrics() {
  ...
  // Create iii with metrics reporting enabled
    ...
    if (result && result.workers) {
      ...
      for (const worker of result.workers) {
        ...
        if (worker.memory_heap_used != null) {
          console.log('\n  Resource Metrics:')
          console.log(
            `    Memory Heap Used: ${(worker.memory_heap_used / 1_048_576).toFixed(2)} MB`,
          )
          console.log(
            `    Memory Heap Total: ${(worker.memory_heap_total / 1_048_576).toFixed(2)} MB`,
          )
